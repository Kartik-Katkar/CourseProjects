...
def all_menu_items(request):
    if request.method == 'GET':
        ...
        ordering = request.query_params.get('orderby')
...
def cart(request):
    ...
    if request.method == 'GET':
        ...
        item_title = request.data['item'].lower()
        item_quantity = int(request.data['quantity'])
        item = get_object_or_404(MenuItem, title=item_title)
        cart_item = Cart(user=User.objects.get(pk=user_id), menuitem=item, unit_price=item.price,
                         quantity=item_quantity, price=item.price * item_quantity)
        try:
            cart_item.save()
        except:
